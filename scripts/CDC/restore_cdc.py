...
def move_local_disks_to_shared(
    target_ip: str,
    target_user: str,
    password: str,
    vmid: int,
    source_local_storage: str,
    target_shared_storage: str,
    target_port: int,
) -> None:
    """CDC 核心：在目标节点将本地存储磁盘迁回共享存储。"""
    conf = get_vm_config(target_ip, target_user, password, vmid, target_port)
    disk_keys = find_disks_on_storage(conf, source_local_storage)
    if not disk_keys:
        print(f"未发现位于本地存储 {source_local_storage} 的业务磁盘，跳过 CDC move-disk。")
        return

    print(f"将在目标节点执行反向 CDC 迁移：{disk_keys}")
    for disk in disk_keys:
        cmd = f"qm move-disk {vmid} {disk} {target_shared_storage} --delete 1 "
        ...
        if "snapshot" in text.lower() and "delete" in text.lower():
            retry_cmd = f"qm move-disk {vmid} {disk} {target_shared_storage} --delete 0 "
...
def select_boot_disk_from_storage(conf_text: str, storage_name: str) -> str | None:
    disks = find_disks_on_storage(conf_text, storage_name)
...
def set_boot_on_target(
    ...
    """CDC 收尾：设置位于共享存储上的启动盘和启动顺序。"""
    conf_after = get_vm_config(target_ip, target_user, password, vmid, target_port)
    boot_disk = select_boot_disk_from_storage(conf_after, target_shared_storage)
    if not boot_disk:
        print("未找到位于共享存储的可用系统盘，跳过 bootdisk 设置。")
        return
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="ICRA V1.0: CDC 反向迁移脚本（仅目标节点操作）")
...
def main() -> None:
    ...
    try:
        ...
        print("\n=== [阶段 2/4] CDC 反向迁移（本地存储 -> 共享存储）===")
        move_local_disks_to_shared(
            target_ip=wired_ip,
            target_user=args.target_user,
            password=args.password,
            vmid=int(args.vmid),
            source_local_storage=str(args.target_local_storage),
            target_shared_storage=str(args.shared_storage),
